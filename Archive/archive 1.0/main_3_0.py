...
class Charts():
    ...
    def stddev_check(self, output_v):
        ...
        f, ax = plt.subplots()
        ax.plot(self.X_r[:100, 0], label = "Actual")
        ax.plot(self.y_r[output_v,:100,0], label = "Decoder")
        ax.legend(loc = 1)
        plt.close(f)
        return f
...
LEARNING_RATE = 0.005
NEURONS = [int(FEATURE_SIZE / 2), int(FEATURE_SIZE / 4)]
EPOCHS = 50
BATCH_SIZE = 300
...
sess_1 = tf.Session()
model = VariationalAutoencoder(FEATURE_SIZE, NEURONS, LEARNING_RATE)
...
f = visualizer.stddev_check(output_var)
f.show()
visualizer.price_charts()
visualizer.returns_dist(N_OUTPUTS, feature_var)
visualizer.sma_cross(90, 360, feature_var, output_charts)
visualizer.avg_stddev(180, N_OUTPUTS, feature_charts)
visualizer.all_returns_dist(FEATURE_SIZE, output_var)
visualizer.rolling_corr(180, feature_var, output_charts)
visualizer.corr_in_vs_out(180, output_var, 0, 1)
visualizer.spread(output_var, 0, 1)
