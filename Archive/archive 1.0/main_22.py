...
class VariationalAutoencoder():
    ...
    def __init__(self, sess, n_features, lr, n_hidden):
        ...
        self.layer_1_w = tf.Variable(tf.random_normal([self._n_features, self._n_hidden[0]], 
                                                      stddev = tf.sqrt(2/(self._n_features + self._n_hidden[0])),
                                                      seed = self.seed))
        self.layer_1 = tf.nn.relu(tf.matmul(self.inputs, self.layer_1_w))
        
        self.layer_2_w = tf.Variable(tf.random_normal([self._n_hidden[0], self._n_hidden[1]], 
                                                      stddev = tf.sqrt(2/(self._n_hidden[0] + self._n_hidden[1])),
                                                      seed = self.seed))
        self.layer_2 = tf.nn.relu(tf.matmul(self.layer_1, self.layer_2_w))
        
        self.z_hidden_weights = tf.Variable(tf.random_normal([self._n_hidden[1], self._n_hidden[2]], 
                                                             stddev = tf.sqrt(2/(self._n_hidden[1] + self._n_hidden[2])),
                                                             seed = self.seed))
        self.z_hidden = tf.nn.relu(tf.matmul(self.layer_2, self.z_hidden_weights))
...
#        self.z_hidden = tf.nn.relu(tf.matmul(self.inputs, self.z_hidden_weights))
        ...
        self.z_hidden_mean = tf.Variable(tf.random_normal([self._n_hidden[2], self._n_z], 
                                                          stddev = tf.sqrt(2/(self._n_hidden[2] + self._n_z)),
                                                          seed = self.seed))
        self.z_hidden_stddev = tf.Variable(tf.random_normal([self._n_hidden[2], self._n_z], 
                                                            stddev = tf.sqrt(2/(self._n_hidden[2] + self._n_z)),
                                                            seed = self.seed))
        ...
        self.z_samples = tf.random_normal(shape = tf.shape(self.z_stddev), 
                                          stddev = 1,#tf.sqrt(2/(self._n_hidden + self._n_z)),
                                          dtype = tf.float32,
                                          seed = self.seed)
        ...
        self.optimizer = tf.train.AdadeltaOptimizer(self._lr).minimize(self.cost)
